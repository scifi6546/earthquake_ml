    ...
    if(eq_file!="IRISDMC"):
        ...
        for event in arrival_data:
            if event["station"]==data[0].stats["station"]:
                ...
                phase_array=np.array([0])
                    
                if(event["phase"]=='S'):
                    phase_array=np.array([1])
                if(event["phase"]=='P'):
                    phase_array=np.array([2])
...
model = model = tf.keras.models.Sequential([
  ...
  tf.keras.layers.Dense(activation='softmax',units=3)
])
...
print(wf_train.shape)
print(phase_train)
model.fit(wf_train,to_categorical(phase_train),epochs=10)
model.evaluate(wf_train,to_catagorical(phase_train))
